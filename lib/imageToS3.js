...
var _ = require('lodash'),
    fs = require('fs-extra'),
    Q = require('q'),
    imStream = require('imagemagick-stream'),
...
var image = function () {
    ...
    var isLandscape = function(path) {
        var result = im.identify({
    ...
    var uploadToS3 = function(type, file, size) {
        ...
        var ratioLabel = isLandscape(file.path) ? 'x' : '';
        ...
        if (size === 'thumbnail') {
            ...
            resizeFunction = imStream().resize('x100');
        } else if (size === 'list') {
            uploadS3Conf.Key = generateName(uploadS3Conf.Key, 'list');
            resizeFunction = imStream().resize(ratioLabel+'200');
        } else if (size === 'large') {
            uploadS3Conf.Key = generateName(uploadS3Conf.Key, 'large');
            resizeFunction = imStream().resize(ratioLabel+'620');
        }  else {
            resizeFunction = imStream().resize(ratioLabel+'310');
        }
    ...
    var save = function (type, file) {
        ...
        uploadToS3(type, file, false).then(function (imageUrl) {
            result.imageUrl = imageUrl;
            return uploadToS3(type, file, 'thumbnail');
        }).then(function (thumbnailImgUrl) {
            result.thumbnailImgUrl = thumbnailImgUrl;
            return uploadToS3(type, file, 'list');
        }).then(function (listImgUrl) {
            result.listImgUrl = listImgUrl;
            return uploadToS3(type, file, 'large');
        }).then(function (largeImgUrl) {
            result.largeImgUrl = largeImgUrl;
